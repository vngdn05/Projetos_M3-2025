...
def calculate_demographic_data(print_data=True):
    ...
    # Using .get() provides a default value (like 0) if the column '>50K' doesn't exist for a country, avoiding errors.
    country_salary = df.groupby('native-country')['salary'].value_counts(normalize=True).unstack().fillna(0)
    country_percentages = country_salary.get('>50K', 0) * 100
